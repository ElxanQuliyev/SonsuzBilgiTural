...
namespace SonsuzBlog.Areas.AdminPanel.Controllers
{
    public class MeqaleController : Controller
    {
        ...
        {
            var tbl_post = db.tbl_post.Where(q=>q.QebulEdildi==true).OrderByDescending(x => x.PostId).Include(t => t.tbl_category).Include(t => t.tbl_sekil).Include(t => t.tbl_users);
        ...
        public ActionResult Delete(int id)
        {
            ...
            meqale.tbl_etiket.Where(x => x.EtiketId == id);
            foreach (var xs in meqale.tbl_etiket)
            {
                db.tbl_etiket.Remove(xs);
